...
var paths = {
    ...
    corejs: 'js/core.js',
    other: [ 'dep/html5shiv.js', 'video/grey_ff.mp4' /* 其它需要拷贝的文件 */ ]
};
...
gulp.task('buildOther', function() {
    return gulp.src(paths.other)
        ...
        .pipe(foreach(function(stream, file) {
            var name = path.relative('./', file.path);
            return stream
                .pipe(rename(name));
        }))
        .pipe(gulp.dest(paths.build));
...
gulp.task('hash', function() {
    ...
    return gulp.src([paths.build + '/**/*.{html,js,css,png,jpg,jpeg,gif,webp,svg,mp4}', '!' + paths.build + '/dist/**'])
        .pipe(plumber({
            errorHandler: handleError
        }))
